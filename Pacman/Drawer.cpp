...
//Uses the aTextureId to search the right texture from map of textures
void Drawer::Draw(ETextureId aTextureId, const int &aCellX, const int &aCellY) const
{
	if (myTextures.size() > 0 && myTextures.at(aTextureId))
	{
		...
		SDL_RenderCopy(myRenderer, aTexture->texture, &sizeRect, &posRect);
